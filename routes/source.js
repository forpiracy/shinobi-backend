...
router.get('/sourceLink/:id', async (req, res) => {
    // console.time();
    
    const srcId = parseInt(req.params.id);
    const url = `https://hianime.to/ajax/v2/episode/sources?id=${srcId}`;

    try {
        console.log(`Fetching source for sourceId: ${srcId}`)

        const serreq = await axios.get(url, {
            headers: {
        ...
        try {
            ...
            res.status(404).json(`Megacloud extraction failed for srcId: ${srcId}`);
            console.error(error);
        }
    } catch (error) {
        res.status(404).json(`Failed to fetch source for srcId: ${srcId}`);
        console.log(`Failed to fetch source for srcId: ${srcId}`);
        console.error(error);
    }
